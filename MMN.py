...
class System:
    def __init__(self, service_rate, system_id):
    ...
    def handle_jobs(self, the_new_job=None):
        ...
        for this_job in temp_copy_of_jobs_in_sys:
            if this_job.service_start_time <= self.current_time and this_job.status < 2:
                ...
                if DEBUG:
                    print("Time: " + "{:.4f}".format(this_job.service_end_time) + "secs \t\t"                  
                        "System Id: " + str(self.system_id) + " Job Id: " + str(
        ...
        if have_new_job:
            ...
            if DEBUG:
                print("Time: " + "{:.4f}".format(self.current_time) + "secs \t\t"
                    "System Id: " + str(self.system_id) + " Job Id: " + str(
...
class Simulator:
    def __init__(self, arrival_rate, service_rate):
        ...
        self.service_rate = service_rate
        self.systems = [System(service_rate, i) for i in range(0, 5)]
        self.avgCnt = 0
        self.serverUtilRate = 0
        print(" ! " + str(self.systems))
    ...
    def run(self, job_count, queue_max_length):
        ...
        while job_id <= job_count:
            ...
            for system in self.systems:
                if min_queue_cnt > len(system.queue_list):
                    min_queue_cnt = len(system.queue_list)
                    chosen_system = system
            new_job = Job(current_time, job_id, chosen_system.service_rate)
            this_jobs[job_id] = new_job
            new_job.add_and_process_job_queue(chosen_system)

            if not chosen_system.handle_jobs(new_job):
                queue_maxed = True
                break
            current_time += random.expovariate(self.arrival_rate)
            job_id += 1
    ...
    def get_system_usage(self):
        for system in self.systems:
            print("Server usage: " + str(
                (system.current_time - system.idle_time) / system.current_time))
...
if __name__ == '__main__':
    ...
    # waiting cnt
    max_time = max(system.current_time for system in simulator.systems)
    sum_area = sum(system.avgCntArea for system in simulator.systems)
    print("Average waiting num: " + str(sum_area / max_time))

    # waiting time
    plot_simulation_delay_time_per_job(jobs, lamda, mu, summary_results)

    # server rate
    simulator.get_system_usage()
